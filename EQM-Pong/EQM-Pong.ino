/******************************************************************* 
*  EQM Single-Pong - Version 1.0 - December the 3th 2019           * 
*                                                                  *
...
#define Pad_Length 50 //48
#define Pad2_Length 400
#define radius 6
...
void setup() {
  ...
  vga.print_P("EQM Single - Pong |by: Eduardo Q Marques (2019)", 200, 200, true, -1, ESPVGAX_OP_OR, true);
  processInputs (); 
...
void processInputs (){
  ...
  //vga.delay(8);
  potTwoPosition = map(analogRead(0), 40, 980, Pad2_Length/2, 512 - Pad_Length/2); 
  potTwoPosition = constrain(potTwoPosition, Pad2_Length/2 + 1, ESPVGAX_HEIGHT - Pad_Length/2 - 1); 
}
...
void ballPosition(){
  ...
  cx = cx + deltaX;
  cy = cy + deltaY; 
  if ((cx > vgaWidth - 3*radius/2 - 8) && (cy > potTwoPosition - Pad2_Length/2) && (cy < potTwoPosition + Pad2_Length/2)){
     cx = cx - 2*deltaX;
     deltaX = - deltaX;
...
void printScore(){
  ...
  vga.print_P("Fall balls =", 200, 200, true, -1, ESPVGAX_OP_OR, true);
  vga.print_P(bufferL, 232, 200, true, -1, ESPVGAX_OP_OR, true);
  vga.print_P(" ", 250, 200, true, -1, ESPVGAX_OP_OR, true);
...
void gameOver(){
  ...
  scoreL = 0;
  scoreR = 0;
  vga.print_P(" Game Over! Try again! ", 200, 200, true, -1, ESPVGAX_OP_OR, true);
}
